...
plt.close("all")
M = np.array([0,0,0])
with open('ksvm_data.txt') as csvfile:
...
# CHANGE HERE:
sig_values = np.array([1, 0.5, 0.1, 0.01])
for s in range(4):
    sigma = sig_values[s] # parameter used in the Gaussian kernel
    ...
    def kappa_gaussian(X,Y):
        kappa = np.exp(- np.linalg.norm(X - Y)**2 / (2*(sigma**2)))
        return kappa
    
    H = np.zeros((N,N))
    for i in range(N):
        for j in range(N):
            H[i,j] = y[i,0] * y[j,0] * kappa_gaussian(X[i,:],X[j,:])
            
    K = np.zeros((N, N))
    for i in range(N):
        for j in range(N):
            K[i,j] = kappa_gaussian(X[i,:],X[j,:])
    
    #MISSING define other parameters required for the cvxopt solver
    H = np.outer(y,y) * K
    f = -1*np.ones((1,N)).T
    Aeq = y.T#y.reshape(1,N)
    ceq = np.zeros(1)# 
    lb = np.zeros(N)
    ub = np.ones(N)*C
    ...
    idx1 = (lambd>1e-9) # was 1e-9
    idx2 = (lambd<(C - 1e-9)) # was 1e-9
    idx = (idx1==idx2).ravel()
    
    print np.where(idx)
    #%%
    # MISSING: compute b (see equation (8.55) in the notes on kernel SVM)
    x_idx = np.where(idx)[0]
    N_supp = np.where(idx)[0].shape[0]
    outer_sum = 0
    for i in range(N_supp):
        inner_sum = 0
        for j in range(N_supp):
            inner_sum = inner_sum + lambd[j,0] * y[j,0] * kappa_gaussian(X[x_idx[i],:],X[x_idx[j],:])
        outer_sum = inner_sum - y[i,0]
    b = float(1)/float(N_supp) * float(outer_sum)
